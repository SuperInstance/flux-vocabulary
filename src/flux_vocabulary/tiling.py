...
class TilingSystem:
    ...
    def _load_base(self):
        ...
        # Try multiple possible locations for vocabulary data files
        search_paths = [
            # Relative to this package's data
        ...
        for base in search_paths:
            cp = os.path.join(base, 'core')
            mp = os.path.join(base, 'math')
            if os.path.isdir(cp):
                core_path = cp
            if os.path.isdir(mp):
                math_path = mp
    ...
    def run(self, text: str) -> TileResult:
        ...
        if match:
            ...
            if result:
                ...
                return TileResult(
                    success=False,
                    error="Bytecode execution unavailable (standalone mode)",
                )
    ...
    def _run_bytecode(self, asm: str, result_reg: int):
        """Assemble and execute bytecode in sandbox. Returns None if unavailable."""
        if assemble_text is None or SandboxVM is None:
            return None
        try:
...
def build_default_tiling() -> 'TilingSystem':
    """
...
TilingInterpreter = TilingSystem
