...
to other agents. But at compile time, unused vocabulary gets pruned
to produce a minimal runtime optimized for the actual hardware.

Like a hermit crab: carry everything when you're exploring,
pack only what you need when you move to a new shell.

Usage:
    from flux_vocabulary.pruning import UsageTracker, VocabularyPruner, RuntimeCompiler
...
class VocabularyPruner:
    ...
    3. By hardware: keep entries suitable for target hardware
    """

    def prune(self, vocab, tracker: UsageTracker, min_calls: int = 1):
        ...
        # Build a new vocabulary-like object
        pruned = type(vocab)()
        for entry in vocab.entries:
    ...
    def prune_to_size(self, vocab, tracker: UsageTracker, max_entries: int = 100):
    ...
    def prune_for_hardware(self, vocab, tracker: UsageTracker,
        ...
        - "gpu": only compute-heavy entries
        """
        if target == "embedded":
            pruned = type(vocab)()
            for entry in vocab.entries:
...
class RuntimeCompiler:
    ...
    Generates a single .py file with minimal VM, assembler, vocabulary
    entries, and a run() function. Zero dependencies.
    """
